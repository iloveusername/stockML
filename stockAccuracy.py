...
def getPrice(tickerName, statesList):
    ...
    # print(priceHistory)
    randomInt = random.randint(1011, len(priceHistory)-15)
    priceHistory = list(priceHistory['Open'])
    y = priceHistory[randomInt]
    priceHistory = priceHistory[randomInt - 1010:randomInt-10]
    ...
    return priceHistory, currentPrice, y
...
averages = []
for x in range(10):
    priceHistory, currentPrice, y = getPrice(ticker, states)
    model.eval()
    with torch.no_grad():
        prediction = model(priceHistory).numpy()[0]
        prediction = prediction.item()
        difference = prediction - currentPrice
        averages.append((y-prediction)/prediction)
        print()
        print('#####################')
